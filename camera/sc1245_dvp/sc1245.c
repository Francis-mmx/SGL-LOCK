...
extern void sc1245_dvp_ae_ev_init(u32 fps);
...
unsigned char wr_sc1245_reg(u16 regID, unsigned char regDat)
{
    u8 ret = 1;
...
unsigned char rd_sc1245_reg(u16 regID, unsigned char *regDat)
{
    u8 ret = 1;
    dev_ioctl(iic, IIC_IOCTL_START, 0);
    if (dev_ioctl(iic, IIC_IOCTL_TX_WITH_START_BIT, WRCMD)) {
...
/*************************************************************************************************
    sensor api
*************************************************************************************************/
static u32 sc1245_frame_rate = 30;
void sc1245_dvp_config_SENSOR(u16 *width, u16 *height, u8 *format, u8 *frame_freq)
{
    u32 i;
    u8 v;

    rd_sc1245_reg(0x3020, &dType);
    printf("1253 ver:%d\n", dType);
    sc1245_dvp_set_output_size(width, height, frame_freq);

    if (0x03 == dType) {
        for (i = 0; i < sizeof(SC1245_INI_REG03) / sizeof(sensor_ini_regs_t); i++) {
            wr_sc1245_reg(SC1245_INI_REG03[i].addr, SC1245_INI_REG03[i].value);
        }
    }
    if (0x02 == dType) {
        for (i = 0; i < sizeof(SC1245_INI_REG02) / sizeof(sensor_ini_regs_t); i++) {
            wr_sc1245_reg(SC1245_INI_REG02[i].addr, SC1245_INI_REG02[i].value);
        }
    }
    ...
    /* wr_sc1245_reg(0x320e, 0x02); */
    /* wr_sc1245_reg(0x320f, 0xee); */
    /* }else if(sc1245_frame_rate==25){ */
    /* //output format 36M 1600X900 25fps */
    /* wr_sc1245_reg(0x320e, 0x03); */
    /* wr_sc1245_reg(0x320f, 0x84); */
    /* }else if(sc1245_frame_rate==33){ */
    /* //output format 36M 1600X750 33.3fps */
    wr_sc1245_reg(0x320c, 0x06);
    wr_sc1245_reg(0x320d, 0x40);
    wr_sc1245_reg(0x320e, 0x02);
    wr_sc1245_reg(0x320f, 0xEE);
    /* } */
#else
...
#endif
    sc1245_dvp_ae_ev_init(*frame_freq);
...
s32 sc1245_dvp_set_output_size(u16 *width, u16 *height, u8 *frame_freq)
{
    return 0;
}
...
s32 sc1245_dvp_ID_check(void)
{
    ...
    for (i = 0; i < 3; i++) { //
        rd_sc1245_reg(0x3107, &pid);
        rd_sc1245_reg(0x3108, &ver);
    }
...
s32 sc1245_dvp_check(u8 isp_dev, u32 reset_gpio, u32 pwdn_gpio)
{
...
s32 sc1245_dvp_init(u8 isp_dev, u16 *width, u16 *height, u8 *format, u8 *frame_freq)
{
    ...
    sc1245_dvp_config_SENSOR(width, height, format, frame_freq);
...
static void updateforgain02(u32 dgain)
{
    u8 data2, data1;
    ...
    wr_sc1245_reg(0x3903, 0x84);
    wr_sc1245_reg(0x3903, 0x04);
    wr_sc1245_reg(0x3812, 0x00);

    if (data2 == 0) { //<2
        wr_sc1245_reg(0x3633, 0x42); //20171212
        wr_sc1245_reg(0x3301, 0x05); //20171106 [3,4]
        wr_sc1245_reg(0x3622, 0xd6);
        updateforshutter();
        wr_sc1245_reg(0x3812, 0x30);
        return;
    }
    if (data2 <= 4) { //<4
        wr_sc1245_reg(0x3633, 0x42); //20171212
        wr_sc1245_reg(0x3301, 0x06); //20171106 [3,4]
        wr_sc1245_reg(0x3622, 0xd6);
        updateforshutter();
        wr_sc1245_reg(0x3812, 0x30);
        return;
    ...
    if (data2 <= 0x1c) { //<15.5
        wr_sc1245_reg(0x3633, 0x42); //20171212
        wr_sc1245_reg(0x3301, 0x07); //20171106 [3,4]
        wr_sc1245_reg(0x3622, 0x16);
        updateforshutter();
        wr_sc1245_reg(0x3812, 0x30);
        return;
    }
    if (data1 == 0x1f) { //>=15.5
        wr_sc1245_reg(0x3633, 0x42); //20171212
        wr_sc1245_reg(0x3301, 0x32); //20171106 [3,4]
        wr_sc1245_reg(0x3622, 0x16);
        updateforshutter();
        wr_sc1245_reg(0x3812, 0x30);
        return;
...
static void updateforgain03(u32 gain)
{
    wr_sc1245_reg(0x3903, 0x84);
    wr_sc1245_reg(0x3903, 0x04);
    wr_sc1245_reg(0x3812, 0x00);
    ...
    if (gain < 2) {
        wr_sc1245_reg(0x3301, 0x07);
        wr_sc1245_reg(0x3631, 0x84);
        wr_sc1245_reg(0x3620, 0x28);
        wr_sc1245_reg(0x3812, 0x30);
        return;
    }
    if (gain < 4) {
        wr_sc1245_reg(0x3301, 0x08);
        wr_sc1245_reg(0x3631, 0x86);
        wr_sc1245_reg(0x3620, 0x08);
        wr_sc1245_reg(0x3812, 0x30);
        return;
    }
    if (gain < 8) {
        wr_sc1245_reg(0x3301, 0x09);
        wr_sc1245_reg(0x3631, 0x86);
        wr_sc1245_reg(0x3620, 0x08);
        wr_sc1245_reg(0x3812, 0x30);
        return;
    }
    wr_sc1245_reg(0x3301, 0x20);
    wr_sc1245_reg(0x3631, 0x86);
    wr_sc1245_reg(0x3620, 0x08);
    wr_sc1245_reg(0x3812, 0x30);
...
//    wrSC1245Reg(0x3e09, gainval & 0xff);
    ...
    wr_sc1245_reg(0x3e08, ((again >> 14) & 0xff));
    wr_sc1245_reg(0x3e09, (again >> 6) & 0xff);
    if (0x03 == dType) {
        updateforgain03(again / 1024);
    }
    if (0x02 == dType) {
        updateforgain02(again);
    }
    return;
}
...
static void set_dgain(u32 dgain, u32 gain)
{
    ...
    wr_sc1245_reg(0x3e0f, 0x84 | ((dgain << 5) & 0xf0)); // ppk add dgain on 20151015
}
...
//
//        wr_sc1245_reg(0x320c, line_length_clk >> 8);
//        wr_sc1245_reg(0x320d, (line_length_clk  & 0xff));
//    }
    ...
    wr_sc1245_reg(0x3e01, texp >> 4);
    wr_sc1245_reg(0x3e02, (texp << 4) & 0xf0);
}
...
u32 sc1245_dvp_set_shutter(isp_ae_shutter_t *shutter)
{
    /* static int ffff = 0; */
    /* static u8 aaaa = 0; */
...
void sc1245_dvp_wr_reg(u16 addr, u16 val)
{
    printf("update reg%x with %x\n", addr, val);
    wr_sc1245_reg((u16)addr, (u8)val);
}
u16 sc1245_dvp_rd_reg(u16 addr)
{
    u8 val;
    rd_sc1245_reg((u16)addr, &val);
    return val;
}
