...
#include "gpio.h"
#include "sc2143_mipi.h"
#include "asm/isp_alg.h"

static u32 reset_gpios[2] = {-1, -1};
static u32 pwdn_gpios[2] = {-1, -1};

extern void *sc2143_mipi_get_ae_params();
extern void *sc2143_mipi_get_awb_params();
extern void *sc2143_mipi_get_iq_params();
extern void sc2143_mipi_ae_ev_init(u32 fps);
...
const Sensor_reg_ini sc2143_mipi_INI_REG[] = {
...
unsigned char wrSc2143_mipi_Reg(u16 regID, unsigned char regDat)
{
    u8 ret = 1;
...
unsigned char rdSc2143_mipi_Reg(u16 regID, unsigned char *regDat)
{
    u8 ret = 1;
    dev_ioctl(iic, IIC_IOCTL_START, 0);
    if (dev_ioctl(iic, IIC_IOCTL_TX_WITH_START_BIT, WRCMD)) {
...
void sc2143_mipi_config_SENSOR(u16 *width, u16 *height, u8 *format, u8 *frame_freq)
{
    u32 i;
    u8 v;


    sc2143_mipi_set_output_size(width, height, frame_freq);

    for (i = 0; i < sizeof(sc2143_mipi_INI_REG) / sizeof(Sensor_reg_ini); i++) {
        wrSc2143_mipi_Reg(sc2143_mipi_INI_REG[i].addr, sc2143_mipi_INI_REG[i].value);
    }
    /*for(i=0;i<sizeof(sc2143_mipi_INI_REG)/sizeof(Sensor_reg_ini);i++)
    {
        rdSc2143_mipi_Reg(sc2143_mipi_INI_REG[i].addr,&v);
        printf("0x%4x,0x%2x\n", sc2143_mipi_INI_REG[i].addr, v);
    }*/

    sc2143_mipi_ae_ev_init(*frame_freq);
...
s32 sc2143_mipi_set_output_size(u16 *width, u16 *height, u8 *frame_freq)
{
    *width = SC2143_MIPI_OUTPUT_W;
    *height = SC2143_MIPI_OUTPUT_H;
...
s32 sc2143_mipi_ID_check(void)
{
    ...
    for (i = 0; i < 3; i++) { //
        rdSc2143_mipi_Reg(0x3107, &pid);
        rdSc2143_mipi_Reg(0x3108, &ver);
    }
...
void sc2143_mipi_reset(u8 isp_dev)
{
    u32 reset_gpio;
    u32 pwdn_gpio;
...
s32 sc2143_mipi_check(u8 isp_dev, u32 reset_gpio, u32 pwdn_gpio)
{

    puts("sc2143_mipi_check\n");
    if (!iic) {
        if (isp_dev == ISP_DEV_0) {
            iic = dev_open("iic0", 0);
        } else {
    ...
    sc2143_mipi_reset(isp_dev);

    if (0 != sc2143_mipi_ID_check()) {
        dev_close(iic);
        iic = NULL;
        return -1;
    }
...
s32 sc2143_mipi_init(u8 isp_dev, u16 *width, u16 *height, u8 *format, u8 *frame_freq)
{
    ...
    sc2143_mipi_config_SENSOR(width, height, format, frame_freq);
...
static void set_dgain(u32 dgain)
{
    ...
    wrSc2143_mipi_Reg(0x3e08, dgain >> 8);
    wrSc2143_mipi_Reg(0x3e09, dgain & 0xff);
}
...
{
    wrSc2143_mipi_Reg(0x3e01, texp >> 4);
    wrSc2143_mipi_Reg(0x3e02, (texp << 4) & 0xf0);
}


u32 sc2143_mipi_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain)
{
    u32 texp;
    u32 texp_align;
    u32 ratio;
...
u32 sc2143_mipi_set_shutter(isp_ae_shutter_t *shutter)
{
...
void sc2143_mipi_W_Reg(u16 addr, u16 val)
{
    ...
    wrSc2143_mipi_Reg((u16)addr, (u8)val);
}
u16 sc2143_mipi_R_Reg(u16 addr)
{
    u8 val;
    rdSc2143_mipi_Reg((u16)addr, &val);
    return val;
}

REGISTER_CAMERA(sc2143_mipi) = {
    .logo 				= 	"sc2143m",
    ...
    .sen_size 			= 	{SC2143_MIPI_OUTPUT_W, SC2143_MIPI_OUTPUT_H},
    .isp_size 			= 	{SC2143_MIPI_OUTPUT_W, SC2143_MIPI_OUTPUT_H},

    .cap_fps         		= 	30,
    .sen_cap_size 			= 	{SC2143_MIPI_OUTPUT_W, SC2143_MIPI_OUTPUT_H},
    .isp_cap_size 			= 	{SC2143_MIPI_OUTPUT_W, SC2143_MIPI_OUTPUT_H},

    .ops                =   {
        .avin_fps           =   NULL,
        .avin_valid_signal  =   NULL,
        .avin_mode_det      =   NULL,
        .sensor_check 		= 	sc2143_mipi_check,
        .init 		        = 	sc2143_mipi_init,
        .set_size_fps 		=	sc2143_mipi_set_output_size,
        .power_ctrl         =   sc2143_mipi_power_ctl,

        .get_ae_params  	=	sc2143_mipi_get_ae_params,
        .get_awb_params 	=	sc2143_mipi_get_awb_params,
        .get_iq_params 	    	=	sc2143_mipi_get_iq_params,

        .sleep 		        =	sc2143_mipi_sleep,
        .wakeup 		    =	sc2143_mipi_wakeup,
        .write_reg 		    =	sc2143_mipi_W_Reg,
        .read_reg 		    =	sc2143_mipi_R_Reg,
