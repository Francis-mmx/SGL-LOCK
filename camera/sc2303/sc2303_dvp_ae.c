...
#include "sc2303_dvp_ae.h"
#include "sc2303_dvp.h"

#include "asm/isp_alg.h"


extern u32 sc2303_dvp_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 sc2303_dvp_set_shutter(isp_ae_shutter_t *shutter);
extern void  *sc2303_dvp_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);


//Anti-flicker:50Hz
const static u32 sc2303_dvp_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 sc2303_dvp_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    20000,       100,    AE_INTERP_GAIN},
...
const static u32 sc2303_dvp_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 sc2303_dvp_ae_50hz_variable_fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    /*
        {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
        {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    ...
    {13653,    20000,       100,    AE_INTERP_GAIN},
    {6826,     30000,       133,    AE_INTERP_GAIN},
    //{3413,     30000,       266,    AE_INTERP_GAIN},// 30FPS
    {3413,     40000,       200,    AE_INTERP_GAIN},// 25FPS
    {1707,     40000,       400,    AE_INTERP_GAIN},
    {853,      40000,       800,    AE_INTERP_GAIN},
    {427,      40000,      1600,    AE_INTERP_GAIN},
    {213,      40000,      3200,    AE_INTERP_GAIN}, // 25FPS
    {106,      40000,      6400,    AE_INTERP_GAIN},
    {53,       40000,     12800,    AE_INTERP_GAIN},// 25FPS
    {26,       40000,     25600,    AE_INTERP_GAIN},
    // {213,      50000,      2560,    AE_INTERP_GAIN},// 20FPS
    //{106,      50000,      5120,    AE_INTERP_GAIN},
    //{53,       50000,     10240,    AE_INTERP_GAIN},
    //{26,       50000,     20480,    AE_INTERP_GAIN},
    {13,       50000,     40960,    AE_INTERP_GAIN}, // 20FPS

    {6,       60000,     68267,    AE_INTERP_GAIN}, // 15FPS
...
static isp_ae_params_t  sc2303_dvp_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = sc2303_dvp_calc_shutter,
        .set_shutter = sc2303_dvp_set_shutter,
        .get_ae_curve = sc2303_dvp_get_ae_curve,
    },
    .ae_step = 1, // 1 //0 PPK ADD ON 20170726
};


void sc2303_dvp_ae_ev_init(u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = sc2303_dvp_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);

    sc2303_dvp_calc_shutter(&ae_shutter, time, gain);

    sc2303_dvp_set_shutter(&ae_shutter);
}

void   *sc2303_dvp_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    const void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = sc2303_dvp_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
        ...
        *max_ev = AE_MAX_EV;
        ...
        curve = sc2303_dvp_ae_50hz_variable_fps;
#else
        *min_ev = AE_MIN_EV;
        *max_ev = AE_MAX_EV;
        ...
        if (fps == 25) {
            ...
            curve = &sc2303_dvp_ae_50hz_30fps;
        }
        ...
        *max_ev = AE_MAX_EV;
        ...
        curve = sc2303_dvp_ae_60hz_30fps;
...
void *sc2303_dvp_get_ae_params()
{
    return (void *)&sc2303_dvp_ae_params;
}
