...
struct ui_platform_api {
    ...
    int (*get_vscreen_info)(struct draw_context *, void *info);
    ...
    int (*paint_dots)(struct draw_context *, struct brush_set *brush, struct position *array, int num);
...
int ui_persent_to_real_value(int base_value, int persent);
int ui_real_value_to_persent(int base_value, int real_value);

int ui_core_set_elm_invisible(struct element *elm, int invisible);

void ui_core_get_element_abs_rect(struct element *elm, struct rect *rect);

int ui_core_set_elm_position(struct element *elm, int x, int y);
...
void ui_core_element_release(struct element *elm);
...
int ui_core_element_skip_floor(struct element *elm, enum skip_f sk, u8 cnt);
 ...
 * 锁定控件的上下文，先不推向imb显示
 */
int ui_lock_dc(int id);
int ui_unlock_dc(int id);

int ui_core_dc_move(struct element *elm, int step, int updown);


int ui_core_scroll_back();
