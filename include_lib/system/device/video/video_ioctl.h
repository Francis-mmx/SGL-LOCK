...
#include "device/camera_effect.h"
...
#define VIDEO_CROP_ENABNLE		(1<<0)
#define VIDEO_LARGE_IMAGE_ENABNLE	(1<<1) //大分辨率关闭裁剪功能，防止wifiSDK大屏显示带宽不足
...
#define VIDIOC_SCA                      _IOW('V', 26, sizeof(int))
#define VIDIOC_STICKER_SWITCH           _IOW('V', 27, sizeof(int))
#define VIDIOC_CAMERA_SWITCH            _IOW('V', 28, sizeof(int))
#define VIDIOC_STOP_DIMC 	            _IOW('V', 29, sizeof(int))
#define VIDIOC_BUFF_MOVE                _IOW('V', 30, sizeof(int))
...
struct video_format {
    ...
    u8 src_crop_enable;//0:支持通过配置比例对IMC源数据进行裁剪
    u8 sticker_en;
    struct sticker_image_info *image;
    u8 sticker_imc_ch;
    u8 *sticker_name;
};
...
struct video_image_capture {
    ...
    struct image_sticker *icon_sticker[4];
    // 万花筒特效
    struct camera_effect_config *eff_cfg;
    u8 is_rec;
};
