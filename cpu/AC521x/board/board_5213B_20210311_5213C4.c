...
// 顶板丝印 AC5213B_V2.0_20210311
// 实际使用芯片为AC5213C4 8M SDRAM
...
{
#define SD_IO_DECT_PIN -1//IO_PORTH_12
    static u8 init = 0;
    if (!init) {
        init = 1;
...
{
    static u8 init = 0;
#define AVIN_DET_PIN -1//IO_PORTH_13
    if (!init) {
        init = 1;
...
//    gpio_direction_output(IO_PORTB_11, 0);
    ...
    // 手动关闭上拉，否则IOKEY会一直打印key 12
    gpio_set_pull_up(IO_PORTH_12, 0);
    gpio_set_pull_up(IO_PORTH_13, 0);
    gpio_set_pull_down(IO_PORTH_12, 1);
    gpio_set_pull_down(IO_PORTH_13, 1);
}
