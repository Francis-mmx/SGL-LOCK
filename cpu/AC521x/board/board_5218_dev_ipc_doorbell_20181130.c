...
#ifdef CONFIG_BOARD_5218B_DEV_IPC_DOORBELL_20181130
...
int sdmmc_1_io_detect(const struct sdmmc_platform_data *data)
{
    ...
    if (!init) {
        init = 1;
        gpio_direction_input(SD1_DET);
        gpio_set_pull_up(SD1_DET, 1);
        gpio_set_pull_down(SD1_DET, 0);
    }

	return !gpio_read(SD1_DET);
}

static void sdmmc_power(int on)
{
    gpio_direction_output(IO_PORTH_07, !on);
}
...
LCD_PLATFORM_DATA_BEGIN(lcd_data)
	.interface = LCD_MIPI,
	.lcd_io = {
        .backlight = IO_PORTG_05,
        .backlight_value = 1,

		.lcd_reset = IO_PORTG_07,
        .lcd_cs    = -1,
        .lcd_lane  = -1,
        .lcd_rs    = -1,
        .lcd_spi_ck= -1,
...
SW_TOUCH_PANEL_PLATFORM_DATA_BEGIN(touch_panel_data)
    ...
    .int_pin        = IO_PORTF_03,
    ._MAX_POINT     = 1,
    ._MAX_X         = LCD_DEV_HIGHT,
    ._MAX_Y         = LCD_DEV_WIDTH,
    ._INT_TRIGGER   = 3,
    ._X2Y_EN        = 0,
    ._X_MIRRORING   = 0,
    ._Y_MIRRORING   = 1,
...
//#ifdef CONFIG_NET_TCP_ENABLE /*V1.0.6-P4以上,TCP图传需要开轮训,才能解决速度慢问题*/
    .sdio_parm = SDIO_GRP_1 | SDIO_PORT_1 | SDIO_1_BIT_DATA | SDIO_CLOCK_26M,
//#else
...
#ifdef CONFIG_VIDEO2_ENABLE
    //UVC 后拉供电控制
    gpio_direction_output(IO_PORTF_04, 1);
#endif
    //LCD 4LANE
