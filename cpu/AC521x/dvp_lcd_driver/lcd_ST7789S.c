...
#include "asm\dsi.h"
extern u32 get_imb_init_finish();
static void lcd_kick_manual(void *priv)
{
    if (get_imb_init_finish() != 2) {
        return ;
    }

    struct lcd_dev_drive *p = (struct lcd_dev_drive *)priv;
    if (!p) {
        return;
    }
    if (p->bl_ctrl_flags == BL_CTRL_BACKLIGHT_AND_CONTROLER) { //sotp kick after lcd off
        if (lcd_get_bl_sta()) {
            if (p->type == LCD_MIPI) {
                dsi_vdo_kick;
                if (p->te_mode_dbug) {
                    log_d("bl mipi kick 0");
                }
            } else {
                struct imd_dev *dev = (struct imd_dev *)p->dev;
                if (p->setxy) {
                    p->setxy(0, dev->timing.hori_pixel - 1, 0, dev->timing.vert_pixel - 1);
                }
                imd_start();
                if (p->te_mode_dbug) {
                    log_d("bl dvp kick 0");
...
static void lcd_st7789s_init(void *_data)
{
    ...
    if (te_mode.te_mode_en) {
        struct lcd_dev_drive *lcd = lcd_dev_match("st7789s");
        if (lcd) {
            Port_Wakeup_Reg(te_mode.event, te_mode.gpio, te_mode.edge, lcd_kick_manual, lcd);
        }
...
REGISTER_IMD_DEVICE(lcd_st7789s_dev) = {
    .info = {
        .test_mode       = false,
    ...
    .sync0_cfg       = CLK_EN | SIGNAL_TYPE_DEN | CLK_POSITIVE,
...
#else
    .clk_cfg	 	 = IMD_INTERNAL | OSC_32K | PLL1_CLK | DIVA_3 | DIVB_1,
