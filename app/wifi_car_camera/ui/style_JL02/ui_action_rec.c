...
#if (DOUBLE_720 == 1)
const static int _ENC_SET_PIC_C1[] = {
    ENC_SET_PIC_C1_1,
};
#endif
...
const struct car_num text_car_num_table[] = {
...
void enc_menu_show(enum ENC_MENU item)
{
    ...
    case ENC_MENU_RESOLUTION:
...
#if (DOUBLE_720 == 1)
        ui_show(ENC_SET_LAY_C1);
        ui_show(ENC_SET_PIC_SEL_1_1);
        ui_highlight_element_by_id(ENC_SET_PIC_1_1);
        ui_highlight_element_by_id(ENC_SET_TXT_1_1);
        ui_text_show_index_by_id(ENC_SET_TXT_C1_1, 0);
        ui_highlight_element_by_id(_ENC_SET_PIC_C1[__this->resolution]);
        ...
        ui_highlight_element_by_id(ENC_SET_PIC_1_1);
        ui_highlight_element_by_id(ENC_SET_TXT_1_1);
        ui_text_show_index_by_id(ENC_SET_TXT_C2_1, 1);
        ui_text_show_index_by_id(ENC_SET_TXT_C2_2, 1);
        ui_highlight_element_by_id(_ENC_SET_PIC_C2[__this->resolution]);
        __this->enc_menu_status = item;
#endif
        break;
    case ENC_MENU_DUALVIDEO:
        ...
        ui_text_show_index_by_id(ENC_SET_TXT_C2_1, 0);
        ui_text_show_index_by_id(ENC_SET_TXT_C2_2, 0);
    ...
    case ENC_MENU_GAP:
        ...
        ui_text_show_index_by_id(ENC_SET_TXT_C1_1, 1);
        ui_highlight_element_by_id(_ENC_SET_PIC_C1[__this->gap_rec]);
        __this->enc_menu_status = item;
...
static void rec_cnt_handler(int rec_cnt_sec)
{
    ...
    if (if_in_rec == true) {
        rec_cnt = rec_cnt_sec % (cyc_time * 60);
    }
...
static int rec_res_change_handler(const char *type, u32 arg)
{
    int index;
...
static int rec_par_change_handler(const char *type, u32 arg)
{
    if (db_select("par")) {
        ui_show(ENC_PIC_GUARD);
    } else {
...
static const struct uimsg_handl rec_msg_handler[] = {
    ...
    // { NULL, NULL},      /* 必须以此结尾！ */
    { "changeRES",      rec_res_change_handler  },/*改变UI录像分辨率字样*/
    { "changePAR",		rec_par_change_handler  },/*停车监控*/
};
/*
 * (end)
 */
...
static int timer_sys_rec_onchange(void *ctr, enum element_change_event e, void *arg)
{
    ...
    /*switch (e) {*/
    /*case ON_CHANGE_INIT:*/
    get_sys_time(&sys_time);//修复更新系统RTC时间，UI时间不刷新BUG
 ...
 *********************************************************************************/
#if DOUBLE_720
static int menu_enc_c1_ontouch(void *_ctr, struct element_touch_event *e)
{
    UI_ONTOUCH_DEBUG("**c1 ontouch**");
    struct intent it;
    int sel_item = 0;
    ...
    case ELM_EVENT_TOUCH_UP:
        UI_ONTOUCH_DEBUG("ELM_EVENT_TOUCH_UP\n");
        for (int i = 0; i < 1; i++) {
            if (ctr->elm.id == _ENC_SET_PIC_C1[i]) {
                sel_item = i;
                break;
            }
        }
        ...
        case ENC_MENU_RESOLUTION:
            ...
            ui_no_highlight_element_by_id(_ENC_SET_PIC_C1[__this->resolution]);
            ui_highlight_element_by_id(_ENC_SET_PIC_C1[sel_item]);
...
REGISTER_UI_EVENT_HANDLER(ENC_SET_PIC_C1_1)
.ontouch = menu_enc_c1_ontouch,
};
#endif
...
/*************************************************************************************/
/**************************************曝光补偿控件动作***************************************/
#define SLID_X    (820 * LCD_DEV_WIDTH / 1280) //滑块x起始绝对坐标
