    ...
    if (touch_num > 0) {
        ...
        for (i = 0; i < touch_num; i++) {
            ...
            if (_touch_panel_data->_X2Y_EN) {
                ...
                point[tid].y = point_data[8 * i + 3] << 8 | point_data[8 * i + 2];
                point[tid].x = point_data[8 * i + 5] << 8 | point_data[8 * i + 4];
            }
            point[tid].size = point_data[8 * i + 7] << 8 | point_data[8 * i + 6];

            if (_touch_panel_data->_MAX_X != 480) {
                point[tid].x = (int)point[tid].x * _touch_panel_data->_MAX_X / 480;
            }
            if (_touch_panel_data->_MAX_Y != 1280) {
                point[tid].y = (int)point[tid].y * _touch_panel_data->_MAX_Y / 1280;
            }
