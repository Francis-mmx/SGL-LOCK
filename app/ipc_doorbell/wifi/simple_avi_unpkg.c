...
//    put_buf(&avi_info->file_head, sizeof(UNPKG_JL_AVI_HEAD));
    avi_info->flen = flen(fd);
    avi_info->fd = fd;
    avi_info->flag = true;

    printf("\n avi_info->file_head.isft_size = %d\n", avi_info->file_head.isft_size);
//    put_buf(&avi_info->file_head.isft,avi_info->file_head.isft_size);
//    put_buf(&avi_info->file_head.info_head_junk,sizeof(avi_info->file_head.info_head_junk));
//    printf("\n avi_info->file_head.info_junk_len = %d\n",avi_info->file_head.info_junk_len);
//    put_buf(&avi_info->file_head,sizeof(UNPKG_JL_AVI_HEAD));
    fseek(fd, 0, SEEK_SET);
    return 0;
}
...
static u32 avi_find_index_addr(FILE *fd, u8 state)
{
    ...
    u8 try_find_idx = 0; //只在文件末尾找一次idx1
...
get_data:
    ...
    printf("\n avi_info->file_head.len = 0x%x\n", avi_info->file_head.len);
...
//    put_buf(&idex1_head, sizeof(struct idex1_str));
...
//    put_buf(&idex1_head.str ,4);
    if (idex1_head.str != idx1) {
        if (!try_find_idx) {
            /* 兼容avi修复的文件解码 */
            addr = flen(fd) - 512 * 1024 + 8;
            try_find_idx = 1;
            goto get_idx_data;
        }
        fseek(fd, 0, SEEK_SET);
    ...
    printf("\n[ debug ]--func=%s line=%d idx1_addr=%x\n", __func__, __LINE__, avi_info->idx1_addr);
...
//        put_buf(&index_info, index_info_len);
        if (index_info.dwChunkId == 0 || index_info.dwOffset == 0 /*|| index_info.dwSize == 0*/
            || (index_info.dwChunkId != IDX_00DC && index_info.dwChunkId != IDX_01WB && index_info.dwChunkId != IDX_00WB)) {
            break;
        }
    ...
    printf("avi_info->video_chunk_num = %d avi_info->audio_chunk_num  : %d \n", avi_info->video_chunk_num, avi_info->audio_chunk_num);
    return num;
}
...
//        idx_offset = index_info.dwOffset + 0x204;
        idx_offset = index_info.dwOffset + sizeof(UNPKG_JL_AVI_HEAD) + 4;
...
void avi_unpkg_decoder_to_yuv_example(void)
{
    ...
    sprintf(name, "%stest .AVI", CONFIG_ROOT_PATH);
    fd = fopen(name, "r");
    if (!fd) {
