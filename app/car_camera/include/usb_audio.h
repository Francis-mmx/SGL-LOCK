...
#define     SPEAKER_CFG_LEN       	        (8+0x65) 	//AC-S(0x9) + AC-C(0x9) + IT(0xC) + FU(0xA) + OT(0x9) + AS-S0(0x9) + AS-S1(0x9) + AS-C(0x7) + AS-C(0xB) +
#define     SPEAKER_INF_NUM       	        (2)
...
#define		MIC_SUBFRAME_SIZE           (MIC_AUDIO_RES / 8)
#define		MIC_CHANNEL			        1
#define		MIC_FRAME_LEN		        ((MIC_AUDIO_RATE * MIC_SUBFRAME_SIZE * MIC_CHANNEL)/10)
...
//#define     MIC_CFG_LEN       		     (8+0x64)
#define     MIC_INF_NUM       		    (2)
#define     MIC_AC				        (0x27)
...
void uac_mic_setup(void *slave);
void uac_mic_setup_endpoint(void *slave);
void uac_mic_setup_endpoint_rx(void *slave);
u32 uac_mic_descriptor_init(void *slave, u8 *ptr, u32 *inf);
void uac_spk_setup(void *slave);
u32 uac_spk_descriptor_init(void *slave, u8 *ptr, u32 *inf);
u32 uac_spk_setup_rx(void *slave);
void usb_speaker_transfer(void *slave);
#ifdef __cplusplus
}
#endif
